import React, { useState } from 'react'
import { MdOutlineCancel } from 'react-icons/md'
import { BsCheck } from 'react-icons/bs'
import { TooltipComponent } from '@syncfusion/ej2-react-popups'
import { themeColors } from '../data/dummy'

import { useStateContext } from '../contexts/ContextProvider'
import Header from './Header'

const ThemeSettings = () => {
  const { currentColor,themeSettings,currentMode, setThemeSettings,
    setMode,setColor} = useStateContext();

  return (
    <div
      className='bg-half-transperent w-screen 
    fixed nav-item top-0 right-0'
    >
      <div className='dark:bg-secondary-dark-bg dark:text-white float-right p-2 bg-white border h-screen 
      dark:text-gray-200 dark:[#484B52] w-400'>
        <div className='flex justify-between items-center p-4 ml-4'>
         
            <Header title="Settings" />
          
          <button type='button'
            onClick={() => setThemeSettings(false)}
            style={{ color: 'rgb(153,171,180)', borderRadius: '50%' }}
            className='text-2xl p-3 hover:drop-shadow-xl 
          hover:bg-light-gray'
          >
            <MdOutlineCancel />

          </button>
        </div>
        <div className='flex-col  border-t-1 border p-4 ml-4'>
          <p className='font-semibold text-lg'>Theme Options</p>
          <div className='mt-4'>
            <input
              type='radio'
              id='light'
              name="theme"
              className='cursor-pointer'
              value="Light"
              onChange={setMode}
              checked={currentMode==="Light"}
            />
            <label htmlFor='light' className='ml-2 text-md cursor-pointer'>
              Light
            </label>
          </div>
          <div className='mt-4'>
            <input
              type='radio'
              id="dark"
              value="Dark"
              name="theme"
              checked={currentMode==="Dark"}
              onChange={setMode}
            />
            <label htmlFor='dark' className='ml-2 text-md cursor-pointer'>
              Black
            </label>

          </div>
        </div>
        <div className='flex-col  border-t-1 border p-4 ml-4'>
          <p className='font-semibold text-lg'>Theme Colors</p>

          <div className='mt-4 flex gap-3'>
            {
              themeColors.map((item, index) => (
                <TooltipComponent key={index}
                  position='TopCenter'
                >
                  <div className='relative mt-2 cursor-pointer
                flex gap-5 items-center'>
                    <button
                      type='button'
                      className='h-10 w-10 rounded-full  cursor-pointer
                  hover:bg-light-gray'
                      style={{ backgroundColor: item.color }}
                      onClick={() => {setColor(item.color)
                        console.log(currentColor)
                      }} 
                      >
                      
                      
                      <BsCheck 
                      
                      className={`ml-2 text-2xl text-white 
                  ${item.color === currentColor ? "block" :
                          "hidden"}`}
                        />

                    </button>
                  </div>

                </TooltipComponent>
              ))
            }
          </div>

        </div>

      </div>


    </div>
  )
}

export default ThemeSettings